...
async def create_suggestion(
    suggestion: SuggestionCreate, current_user=Depends(get_current_user)
):
    print(current_user)
    return await manager.create_suggestion(
        suggestion.title,
...
async def get_suggestion(
    suggestion_id: int, current_user=Depends(get_current_user)
):
