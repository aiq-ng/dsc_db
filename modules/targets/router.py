...
from modules.targets.models import TargetCreate, TargetResponse, TargetUpdate
...
async def create_target(
    target: TargetCreate, current_user=Depends(get_current_user)
):
...
async def search_targets(
    ...
    if not target_name and not target_number:
        raise HTTPException(
            status_code=400,
            detail="At least one search parameter (target_name or number) is required",
        )
